...
function knitzhub_ftn_optionsframework_add_admin(){
	global $query_string;
	$themename = knitzhub_ftn_get_option('of_themename');
	$shortname = knitzhub_ftn_get_option('of_shortname');
	if(isset($_REQUEST['page'])&& $_REQUEST['page'] == 'optionsframework'){
		if(isset($_REQUEST['of_save'])&& 'reset' == $_REQUEST['of_save']){
			$options = knitzhub_ftn_get_option('of_template');
			knitzhub_ftn_reset_options($options,'optionsframework');
			header("Location: admin.php?page=optionsframework&reset=true");
			die;
			}
		}
	$of_page = add_theme_page($themename,'Option Panel','edit_theme_options','optionsframework','knitzhub_ftn_optionsframework_options_page','div');
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page",'knitzhub_ftn_load_only');
	}
add_action('admin_menu','knitzhub_ftn_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */
function knitzhub_ftn_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;
	...
	if($page == 'optionsframework'){
		knitzhub_ftn_delete_option('of_options');
		}
	}
/* ----------------------------------------------------------------------------------- */
/* Build the Options Page - optionsframework_options_page */
/* ----------------------------------------------------------------------------------- */
function knitzhub_ftn_optionsframework_options_page(){
	$options = knitzhub_ftn_get_option('of_template');
	$themename = knitzhub_ftn_get_option('of_themename');
	?>
    ...
    $return = knitzhub_ftn_optionsframework_machine($options);
    ?>
        <div id="main">
          <div id="of-nav">
            <ul>
...
/* ----------------------------------------------------------------------------------- */
function knitzhub_ftn_load_only(){
	add_action('admin_head','knitzhub_ftn_admin_head');
...
function knitzhub_ftn_admin_head(){
	// COLOR Picker ?>
		...
		foreach($options as $option){
			if($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border'){
				if($option['type'] == 'typography' OR $option['type'] == 'border'){
					$option_id = $option['id'];
					$temp_color = knitzhub_ftn_get_option($option_id);
					$option_id = $option['id'].'_color';
					$color = $temp_color['color'];
					}else{
					$option_id = $option['id'];
					$color = knitzhub_ftn_get_option($option_id);
					}
				?>
...
<?php }
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - knitzhub_ftn_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action','knitzhub_ftn_ajax_callback');
function knitzhub_ftn_ajax_callback(){
	...
	if($save_type == 'upload'){
		...
		knitzhub_ftn_update_option($clickedID,$uploaded_file['url']);
		...
		foreach($options as $option_array){
			$id = $option_array['id'];
			$old_value = knitzhub_ftn_get_option($id);
			$new_value = '';
			if(isset($output[$id])){
				...
				if(is_array($type)){
					foreach($type as $array){
						if($array['type'] == 'text'){
							...
							knitzhub_ftn_update_option($id,stripslashes($new_value));
							}
						}
...
/* ----------------------------------------------------------------------------------- */
function knitzhub_ftn_optionsframework_machine($options){
	$counter = 0;
	$menu = '';
	$output = '';
	foreach($options as $value){
		...
		switch($value['type']){
			...
			case 'text':
			$val = $value['std'];
			$std = knitzhub_ftn_get_option($value['id']);
			if($std != ""){
				$val = $std;
				}
			...
			$value['std'] = '';
			if(isset($value['std'])){
				$output .= knitzhub_ftn_optionsframework_uploader_function($value['id'],$value['std'],null);
				}
			break;
			/******************/
			case "upload_min":
			$output .= knitzhub_ftn_optionsframework_uploader_function($value['id'],$value['std'],'min');
			break;
			/******************/
			case "color":
			$val = $value['std'];
			$stored = knitzhub_ftn_get_option($value['id']);
			if($stored != ""){
				$val = $stored;
		...
		if(is_array($value['type'])){
			foreach($value['type'] as $array){
				$id = $array['id'];
				$std = $array['std'];
				$saved_std = knitzhub_ftn_get_option($id);
				if($saved_std != $std){
					$std = $saved_std;
					}
				$meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - knitzhub_ftn_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */
function knitzhub_ftn_optionsframework_uploader_function($id,$std,$mod){
	$uploader = '';
	$upload = knitzhub_ftn_get_option($id);
	if($mod != 'min'){
		$val = $std;
		if(knitzhub_ftn_get_option($id)!= ""){
			$val = knitzhub_ftn_get_option($id);
			}
